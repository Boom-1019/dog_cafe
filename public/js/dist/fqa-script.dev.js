...
$('.right-arrow').on('click', function () {
  if (currentLeftNum == -1140) {} else {
    // 每點一次箭頭會扣500
    currentLeftNum = currentLeftNum - 1140;
    console.log(currentLeftNum); // 第二層 動畫 left位移一張圖片寬
...
$('.left-arrow').on('click', function () {
  if (currentLeftNum == 0) {//假如currentLeftNum==0，執行這裡
  } else {
    //假如currentLeftNum!=0，執行這裡
    // 每點一次箭頭會扣500
    currentLeftNum = currentLeftNum + 1140; // console.log(currentLeftNum)
    // 第二層 動畫 left位移一張圖片寬
...
$(window).resize(function () {
  gsapAnimate();
  console.log($(window).width()); // widthWidth 在每次縮放視窗寬度時，換上當前的視窗寬度的數值
